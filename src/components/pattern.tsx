...
export const PatternComponent: FC = () => {
  ...
  const handleClick = (row: number, col: number, mouseOver?: boolean) => {
    savePattern({
      ...
      pattern: patternState.pattern.map((r, rowI) =>
        rowI === row
          ? r.map((c, colI) => (colI === col ? getnewcell(c, mouseOver) : c))
          : r
      ),
    });
  };
  ...
  const addCol = (at: number) => {
    savePattern({
      ...
      pattern: patternState.pattern.map((r) => [
        ...r.slice(0, at + 1),
        { colorindex: patternState.selectedColorIndex, type: CELL_TYPE.EMPTY },
        ...r.slice(at + 1),
      ]),
    });
  };
  ...
  const deleteCol = (col: number) => {
    if (!window.confirm("Do you really want to delete whole column?")) return;
    savePattern({
      ...patternState,
      pattern: patternState.pattern.map((r) => r.filter((_c, i) => i !== col)),
    });
  };
    ...
    <>
      <Card className="h-100">
        <Card.Header>
          ...
          {patternState.pattern[0] ? patternState.pattern[0].length : 0}
        ...
        <Card.Body className="pattern-container">
          <div
            className="noselect"
            id="pattern"
            style={{ transform: `scale(${patternState.scaleFactor})` }}
          >
            <div className="r">
              ...
              {patternState.pattern[0].map((col, colIndex) => (
                <div
                  ...
                  className="cell"
                  onClick={() => {
                    setDropDownPos({ row: -1, col: colIndex, opened: true });
                  }}
                >
            ...
            {patternState.pattern.map((row, rowIndex) => (
              <div key={`row-${rowIndex}`} className="r">
                <div
                  ...
                  className="cell"
                  onClick={() => {
                    setDropDownPos({ row: rowIndex, col: 0, opened: true });
                  }}
                >
                ...
                <div
                  ...
                  className="cell"
                  onClick={() => {
                    setDropDownPos({ row: rowIndex, col: 9999, opened: true });
                  }}
                >
                  {patternState.pattern.length - rowIndex}
                  {dropDownPos?.opened &&
            ...
            <div className="r">
              ...
              {patternState.pattern[0].map((col, colIndex) => (
                <div
                  ...
                  className="cell"
                  onClick={() => {
                    setDropDownPos({ row: 99999, col: colIndex, opened: true });
                  }}
                >
                  {patternState.pattern[0].length - colIndex}
                  {dropDownPos?.opened &&
