...
export const PatternWindowComponent: FC = () => {
    ...
    const onUp = useCallback(() => {
        setPos((old) => { return { ...old, row: Math.max(0, old.row - 1) } })
    }, [])

    const onDown = useCallback(() => {
        setPos((old) => { return { ...old, row: Math.min(patternState.pattern.length - pos.wy - 1, old.row + 1) } })
    }, [patternState.pattern.length, pos.wy])

    const onLeft = useCallback(() => {
        setPos((old) => { return { ...old, col: Math.max(0, old.col - 1) } })
    }, [])

    const onRight = useCallback(() => {
        setPos((old) => { return { ...old, col: Math.min(patternState.pattern[0].length - pos.wx - 1, old.col + 1) } })
    }, [patternState.pattern, pos.wx])

    const onShrinkWidth = useCallback(() => {
        setPos((old) => { return { ...old, wx: Math.max(MIN_SIZE, old.wx - 1) } })
    }, [])

    const onGrowWidth = useCallback(() => {
        setPos((old) => { return { ...old, wx: Math.min(patternState.pattern[0].length, old.wx + 1) } })
    }, [patternState.pattern])

    const onShrinkHeight = useCallback(() => {
        setPos((old) => { return { ...old, wy: Math.max(MIN_SIZE, old.wy - 1) } })
    }, [])

    const onGrowHeight = useCallback(() => {
        setPos((old) => { return { ...old, wy: Math.min(patternState.pattern.length, old.wy + 1) } })
                ...
                <Card.Body className="pattern-container">
                    <div className='mb-3'>
                        <InputGroup>
                            ...
                            <Form.Control type='number' min={MIN_SIZE} max={patternState.pattern[0].length} value={pos.wx} onChange={(e) => setPos((old) => { return { ...old, wx: Number(e.target.value) } })} />
                            <HoldButton className='btn btn-outline-secondary' onFire={onShrinkWidth}>-</HoldButton>
                            <HoldButton className='btn btn-outline-secondary' onFire={onGrowWidth}>+</HoldButton>
                            
                            <InputGroup.Text>height</InputGroup.Text>
                            <Form.Control type='number' min={MIN_SIZE} max={patternState.pattern.length} value={pos.wy} onChange={(e) => setPos((old) => { return { ...old, wy: Number(e.target.value) } })} />
                            <HoldButton className='btn btn-outline-secondary' onFire={onShrinkHeight}>-</HoldButton>
                            <HoldButton className='btn btn-outline-secondary' onFire={onGrowHeight}>+</HoldButton>
                        </InputGroup>
                    </div>
                    <div
                        className="noselect"
                    ...
                    >
                        <HoldButton className='btn-outline-secondary m-3' onFire={onUp}>🔼</HoldButton>
                        <div style={{
                            display: 'flex',
                            flexDirection: 'row',
                            overflow: 'auto'
                        }}>
                            <HoldButton className='btn-outline-secondary m-3' onFire={onLeft}>◀️</HoldButton>
                            ...
                            <HoldButton className='btn-outline-secondary m-3' onFire={onRight}>▶️</HoldButton>
                        </div>
                        <HoldButton className='btn-outline-secondary m-3' onFire={onDown}>🔽</HoldButton>
                    </div>
                </Card.Body>
            </Card>
        </>
