...
const drawPattern = (
  ...
  let rowIndexWidth = Math.ceil(metricsRows.width) + 10
...
function drawGrid(
  ...
  const leftAxisX = rowIndexWidth - 4;
  const rightAxisX = w - rowIndexWidth + 4;
  ...
  for (let c = 0; c < cols; c++) {
    const x = c * cellSize;
    ctx.textAlign = "left";
    ctx.fillText((maxCols - startCol - c).toString(), 4, x + cellSize / 2); // top
    ctx.textAlign = "right";
    ctx.fillText((maxCols - startCol - c).toString(), -h + headerHeight * 2 - 4, x + cellSize / 2); // bottom
  }
