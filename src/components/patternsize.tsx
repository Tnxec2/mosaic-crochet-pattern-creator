...
export const PatternSizeComponent: FC<Props> = () => {
  ...
  const cols = useMemo(
    () =>
      patternState.pattern.length > 0 ? patternState.pattern[0].length : 0,
    [patternState]
  );
  ...
  const changeRows = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (newRowCount > patternState.pattern.length) {
      let rowsToAppend = [];
      while (newRowCount > patternState.pattern.length + rowsToAppend.length) {
    ...
    } else if (newRowCount > 0 && newRowCount < patternState.pattern.length) {
      savePattern({
        ...patternState,
  ...
  const changeCols = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (newColsCount > patternState.pattern[0].length) {
      ...
      savePattern({
        ...patternState,
        pattern: patternState.pattern.map((row) => [...row, ...colsToAppend]),
      });
    } else if (
      newColsCount > 0 &&
      newColsCount < patternState.pattern[0].length
      ...
      savePattern({
        ...patternState,
        pattern: patternState.pattern.map((row) => row.slice(0, newColsCount)),
      });
    }
  };
